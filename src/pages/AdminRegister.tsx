...
export default function AdminRegister() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const { data, error } = await supabase.functions.invoke("register", {
      body: { username: username.trim(), password, role: "admin" },
    });
    setSubmitting(false);
    ...
    <div className="min-h-screen bg-background flex flex-col items-center justify-center px-4">
      <div className="w-full max-w-sm space-y-6">
        <div className="text-center space-y-2">
          ...
          <p className="text-sm text-muted-foreground">Request admin access to Dorm Connect</p>
        </div>

        <Card>
          <CardHeader className="pb-4">
            <CardTitle className="text-lg">Register as Admin</CardTitle>
            <CardDescription>Your account will require Primary Admin approval</CardDescription>
          </CardHeader>
          <CardContent>
